...
export const WeatherDisplay = ({
  ...
  return (
    <div className={`relative border-x p-3 sm:p-4 md:p-5 lg:p-6 ${styles.panel} flex flex-col gap-4 sm:gap-5 md:gap-6`}>
      ...
      <div className={`flex flex-col md:flex-row md:items-end justify-between mb-2 sm:mb-3 md:mb-4 relative z-10 border-b pb-3 sm:pb-4 ${styles.border}`}>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 md:gap-5">
        <div className="relative">
          <div className={`text-5xl sm:text-6xl md:text-6xl lg:text-7xl xl:text-7xl font-bold tracking-tighter relative z-10 ${styles.textMain}`} style={{ textShadow: isDayTheme ? 'none' : '4px 4px 0px rgba(8,145,178,0.4)' }}>
            {convertTemp(weather.current.temperature_2m, unit)}°
          </div>
          <div className={`text-xs sm:text-sm md:text-base font-bold tracking-[0.2em] sm:tracking-[0.3em] uppercase mt-1 border-l-4 pl-2 sm:pl-4 ${styles.textHighlight} ${isDayTheme ? 'border-orange-500' : 'border-pink-500'}`}>
        ...
        <div className="relative group flex-shrink-0">
          ...
          <CurrentIcon className={`h-20 w-20 sm:h-24 sm:w-24 md:h-28 md:w-28 lg:h-32 lg:w-32 relative z-10 transition-transform duration-500 group-hover:scale-105 ${CurrentIconData.color}`} strokeWidth={1} />
        </div>
      </div>

      {/* Weather metrics grid: wind, humidity, feels like */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-3 md:gap-4 mt-2 sm:mt-4">
        {/* Wind speed module */}
        <div className={`p-3 sm:p-4 md:p-5 flex flex-col justify-center min-h-[7rem] sm:min-h-[8rem] relative overflow-hidden group transition-colors border ${styles.moduleBg}`}>
          ...
          <span className={`text-[10px] sm:text-xs uppercase tracking-widest mb-2 ${styles.textHighlight}`}>WIND</span>
          <div className="flex items-baseline gap-1 sm:gap-2">
            <span className={`text-3xl sm:text-4xl md:text-5xl font-bold ${styles.textMain}`}>{weather.current.wind_speed_10m}</span>
            <span className={`text-xs sm:text-sm md:text-base ${styles.textAccent}`}>KM/H</span>
          </div>
        </div>
        ...
        <div className={`p-3 sm:p-4 md:p-5 flex flex-col justify-center min-h-[7rem] sm:min-h-[8rem] relative overflow-hidden group transition-colors border ${styles.moduleBg}`}>
          ...
          <span className={`text-[10px] sm:text-xs uppercase tracking-widest mb-2 ${styles.textHighlight}`}>HUMIDITY</span>
          <div className="flex items-baseline gap-1 sm:gap-2">
            <span className={`text-3xl sm:text-4xl md:text-5xl font-bold ${styles.textMain}`}>{weather.current.relative_humidity_2m}</span>
            <span className={`text-xs sm:text-sm md:text-base ${styles.textAccent}`}>%</span>
          </div>
        </div>

        {/* Apparent temperature (feels like) module */}
        <div className={`p-3 sm:p-4 md:p-5 flex flex-col justify-center min-h-[7rem] sm:min-h-[8rem] relative overflow-hidden group transition-colors border ${styles.moduleBg} sm:col-span-2 md:col-span-1`}>
          <div className="absolute top-0 right-0 p-2 sm:p-3 opacity-20 group-hover:opacity-50 transition-opacity">
            <Thermometer className={`h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 ${styles.textAccent}`} />
          </div>
          <span className={`text-[10px] sm:text-xs uppercase tracking-widest mb-2 ${styles.textHighlight}`}>FEELS LIKE</span>
          <div className="flex items-baseline gap-1 sm:gap-2">
            <span className={`text-3xl sm:text-4xl md:text-5xl font-bold ${styles.textMain}`}>{convertTemp(weather.current.apparent_temperature, unit)}</span>
            <span className={`text-xs sm:text-sm md:text-base ${styles.textAccent}`}>°{unit.toUpperCase()}</span>
          </div>
        </div>
      </div>
    </div>
